  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: colors.bgSubtle }]}>
      <View style={styles.content}>
        <Text style={[styles.title, { color: colors.text }]}>Mes bulletins</Text>
        <Text style={[styles.name, { color: colors.textMuted }]}>{user?.name}</Text>
        <Text style={[styles.placeholder, { color: colors.textMuted }]}>
          Mes bulletins et progression — à implémenter (Semaine 5)
        </Text>
      </View>
    </SafeAreaView>
  );
