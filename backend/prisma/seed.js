...
function calcMoy(i1, i2, d, c) {
    return Math.round(((i1 + i2 + d + 2 * c) / 5) * 100) / 100;
}
function appr(m) { return m >= 16 ? 'TB' : m >= 14 ? 'B' : m >= 12 ? 'AB' : m >= 10 ? 'P' : 'F'; }
function ment(a) {
...
async function main() {
    ...
    const adminData = { password: adminPw, name: 'Admin Principal', role: 'ADMIN', institutionId: inst.id };
    ...
    const AY = '2024-2025';
    async function createStudents(defs, classId) {
        const result = [];
    ...
    let recSeq = 1;
    async function assignFee(studentId, fee, paid) {
    ...
    for (let i = 0; i < s6A.length; i++)
        await assignFee(s6A[i].id, fees['6eme'], i < 42);
    for (let i = 0; i < s3B.length; i++)
        await assignFee(s3B[i].id, fees['3eme'], i < 47);
    for (let i = 0; i < s2A.length; i++)
        await assignFee(s2A[i].id, fees['2nde'], i < 45);
    for (let i = 0; i < sTleD.length; i++)
        await assignFee(sTleD[i].id, fees['Terminale'], i < 48);
    console.log('✅ Fees & payments');
    await prisma.$disconnect();
    await prisma.$connect();
    async function seedTerm(students, classId, codes, coefs, teacher, termNumber, termName, publishedAt) {
    ...
    const t1Name = '1er Trimestre';
    const t1Date = new Date('2024-12-20');
    const t2Name = '2ème Trimestre';
    const t2Date = new Date('2025-04-05');
    const t3Name = '3ème Trimestre';
    const t3Date = new Date('2025-07-10');
    ...
    await prisma.$disconnect();
    await prisma.$connect();
    async function seedFiches(classId, codes, teacher, termNumber, termName, signedCount) {
